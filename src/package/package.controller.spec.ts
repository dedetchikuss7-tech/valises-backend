...
describe('PackageController', () => {
  ...
  it('calls reviewContent with actor id, role, package id and dto', async () => {
    const req = {
      user: {
        userId: 'admin-1',
        role: Role.ADMIN,
    ...
    const dto = {
      ...
      contentComplianceNotes: 'Electronics reviewed by admin',
    };

    await controller.reviewContent(req, 'pkg-1', dto);

    expect(packageServiceMock.reviewContent).toHaveBeenCalledWith(
      'admin-1',
      Role.ADMIN,
      'pkg-1',
      dto,
  ...
  it('throws UnauthorizedException when auth role is missing for reviewContent', () => {
    const req = {
      user: {
        userId: 'admin-1',
      },
    ...
    const dto = {
      ...
      contentComplianceNotes: 'ok',
    };

    expect(() => controller.reviewContent(req, 'pkg-1', dto)).toThrow(
      UnauthorizedException,
    );
  });
